# Architecture and deterministic engine guards
...
`FANTASY_QUALITY_RESULT` is an observation, not a substitute for the source report.
...
AST checks reject implicit clocks, network/DB/environment globals, entropy imports,
`Math.random`, aliases/destructuring/dynamic access that conceal Math members, eval and
function constructors. Direct named `createHash` from `node:crypto` is an explicit pure-hash
exception; entropy is not. Explicit seed/PRNG inputs and the physics adapter remain valid.
The policy conservatively reserves relevant identifiers; strings/comments are not code.
It is not a proof of numerical determinism or a complete security sandbox. The engine's
fixed-manifest/physics/OS fixtures remain necessary and are never replaced by graph checks.
