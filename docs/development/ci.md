...
Docs; PRs confined to `apps/web/src` TS/TSX/CSS or `apps/web/index.html` skip the corpus
binding. Main, manual and weekly runs add four `UI (Linux <part>)` jobs
...
comparing all digests. ci-gate's `verify.ts aggregate` binds it (same source, run, attempt,
corpus bytes) to those receipts; its log and `harness-ubuntu-latest` artifact carry the source
report for release, delivery and Issue completion. Local `vp run verify` is unchanged.
...
release. Test results are never cached. Artifacts expire after seven days; PR artifacts
remain data and are never executed by release.
...
Keep `ci-gate`, Security and Dependency policy gates required. Workflow changes do
...
Compare equivalent PR/main runs, including queue, setup and cache.
