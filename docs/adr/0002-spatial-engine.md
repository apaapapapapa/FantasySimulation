...
既存DBの通常起動時削除・自動変換は行わない。初回3D置換後の版更新は
[ADR 0010](0010-battle-version-compatibility.md)に従い、互換な追加では同じDBを使う。
自前resetはADR 0005で廃止した。
...
各試合は最大6000step、障害物256、同時弾64。統合fixtureと実測は[ADR 0011](0011-integrated-performance.md)に記録する。
...
この試作は正式イベント・圧縮・DB・artifact保存・複数Workerを未検証とした。
後続の統合受入は[ADR 0011](0011-integrated-performance.md)を参照する。
...
この結果に基づき、TS+Rapierを**試作採用**する。保存込みの正式採用は後続のADR 0011で判定した。
