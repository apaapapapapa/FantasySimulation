...
import { reconcileBatch } from '@fantasy/api/artifacts';
import { exportPublication } from './publication/publication-export.ts';
import { BattleBundles } from '@fantasy/api/artifacts';
import { readBoundedFile, publishImmutableFile } from '@fantasy/api/artifacts';
...
async function main() {
  ...
  if (command === 'sample' && input && !output) {
    ...
    const { createBatchPlan, executionSource } = await import('@fantasy/api/tooling');
    ...
    const { runBatch } = await import('@fantasy/api/tooling');
    const { executionSource } = await import('@fantasy/api/tooling');
