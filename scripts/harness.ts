...
try {
  ...
  if (!input)
    throw new Error('Usage: harness source <fresh-output> | report <file> <required-check>... | issue-plan <number> | issue-complete <evidence-directory> [--apply]');
  let result;
  ...
  else if (command === 'issue-complete' && (required.length === 0 || (required.length === 1 && required[0] === '--apply')))
