...
await test('complete declaration requires all acceptance evidence and no remaining work', () => {
  ...
  for (const change of [
    { complete: false },
    { remainingWork: ['App authorization pending'] },
    { remainingWork: undefined },
    { acceptance: [] },
    { acceptance: [{ task: 'test', evidence: '' }] },
    { pullRequests: [] },
    { pullRequests: [26, 26] },
    { issue: -1 },
    { issueBodySha256: 'short' },
    { issueUpdatedAt: '2026-02-30T00:00:00Z' },
  ])
...
await test('shared source report and command must describe a clean successful verify of main', () => {
  ...
  for (const change of [
    { sourceSha: mergedSha },
    { testMergeSha: sourceSha, baselineSha: mergedSha },
    { checks: [] },
  ])
...
await test('checklist updates require complete exact coverage including external acceptance', () => {
  ...
  assert.throws(() =>
    completedBody({ ...plan, acceptance: plan.acceptance.slice(1) }, body, sourceSha, 100),
  );
...
await test('fenced examples and HTML comments are not checked off as real tasks', () => {
  const example = '```md\n- [ ] Example\n```\n<!--\n- [ ] Hidden\n-->\n';
  assert.deepEqual(
    issueTasks(example + body).map((entry) => entry.task),
    ['Implementation', 'External setup'],
  );
  assert.ok(
    completedBody(parseCompletion(declaration()), example + body, sourceSha, 100).includes(
      '- [ ] Example',
    ),
  );
...
await test('a verified completion updates body and state in one request and verifies persistence', async () => {
  ...
  assert.equal(
    await completeOne(api, parseCompletion(declaration()), sourceSha, 100, true),
    'UPDATED_AND_CLOSED',
  );
...
await test('dry-run never writes and reopening after completion is respected', async () => {
  const { api, state } = fake();
  assert.equal(
    await completeOne(api, parseCompletion(declaration()), sourceSha, 100, false),
    'DRY_RUN_WOULD_CLOSE',
  );
  ...
  assert.equal(
    await completeOne(api, parseCompletion(declaration()), sourceSha, 100, true),
    'PREVIOUSLY_COMPLETED_DO_NOT_RECLOSE',
  );
  assert.equal(state.writes.length, 0);
});
...
await test('changed Issue body or activity, newer main, unmerged PR and unverified merge all block writes', async () => {
  for (const scenario of [
    ...
    await assert.rejects(() =>
      completeOne(api, parseCompletion(declaration()), sourceSha, 100, true),
    );
    assert.equal(state.writes.length, 0);
  }
});
...
await test('open sub-issues and rejected writes cannot become completion success', async () => {
  ...
  await assert.rejects(
    () => completeOne(fixture.api, parseCompletion(declaration()), sourceSha, 100, true),
    /OPEN_SUB_ISSUES/,
  );
  ...
  await assert.rejects(
    () => completeOne(denied.api, parseCompletion(declaration()), sourceSha, 100, true),
    /403/,
  );
...
await test('job pagination does not silently discard later checks or accept truncated results', async () => {
  ...
  await assert.rejects(() =>
    collection(async () => ({ total_count: 3, jobs: [{}] }), '/jobs', 'jobs'),
  );
});
...
await test('committed completion declarations are explicit and uniquely named', () => {
  ...
  for (const file of readdirSync(directory).filter((file) => file.endsWith('.json'))) {
    const plan = parseCompletion(
      JSON.parse(readFileSync(new URL(file, directory), 'utf8')) as unknown,
    );
...
await test('writer workflow is isolated from PRs and bound to the successful main run', () => {
  const workflow = readFileSync(
    new URL('../../.github/workflows/issue-completion.yml', import.meta.url),
    'utf8',
  );
  assert.ok(workflow.includes('workflow_run:'));
  assert.ok(
    workflow.includes('github.event.workflow_run.head_repository.full_name == github.repository'),
  );
