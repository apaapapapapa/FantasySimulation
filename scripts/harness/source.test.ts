...
describe('source evidence collection', () => {
  it('records a real source identity and the fixed command without recursively verifying tests', async () => {
    ...
    try {
      const result = await collectSource(repo.root, '.generated/harness/success', async (cmd, args) => {
        assert.equal(cmd, 'vp');
        assert.deepEqual(args, ['run', 'verify']);
        return passed;
      }, {});
      assert.equal(result.exitCode, 0);
      const receipt = JSON.parse(readFileSync(join(repo.root, '.generated/harness/success/command.json'), 'utf8')) as Record<string, unknown>;
  ...
  it('does not execute against dirty or untracked source', async () => {
    ...
    try {
      ...
      const result = await collectSource(repo.root, '.generated/harness/dirty', async () => {
  ...
  it('fails if the command changes the source, exits unsuccessfully or exceeds its budget', async () => {
    for (const mode of ['mutation', 'exit', 'budget']) {
      const repo = repository();
      try {
        const result = await collectSource(repo.root, '.generated/harness/fail', async () => {
  ...
  it('refuses stale CI identity and unsafe or reused evidence paths', () => {
    ...
    try {
      ...
      symlinkSync(repo.root, join(repo.root, '.generated/harness/link'), process.platform === 'win32' ? 'junction' : 'dir');
...
describe('bounded execution', () => {
  it('captures success, failure and spawn errors', async () => {
    assert.equal((await runCommand(process.execPath, ['-e', 'console.log("ok")'], tmpdir())).exitCode, 0);
    assert.equal((await runCommand(process.execPath, ['-e', 'process.exit(3)'], tmpdir())).exitCode, 3);
  ...
  it('terminates over-time and over-output commands', async () => {
    const timed = await runCommand(process.execPath, ['-e', 'setInterval(() => {}, 100)'], tmpdir(), { timeoutMs: 200 });
    assert.equal(timed.bounded, true);
    const large = await runCommand(process.execPath, ['-e', 'console.log("x".repeat(10000))'], tmpdir(), { maxBytes: 100 });
  ...
  it('does not forward secrets or execution-injection environment variables', () => {
    assert.deepEqual(safeEnvironment({ PATH: '/tools', GH_TOKEN: 'secret-value', NODE_OPTIONS: '--import evil', CI: 'true' }), { PATH: '/tools', CI: 'true' });
    assert.equal(redact('secret-value ghp_abcdefgh', { GH_TOKEN: 'secret-value' }), '[REDACTED] [REDACTED]');
  });
});
