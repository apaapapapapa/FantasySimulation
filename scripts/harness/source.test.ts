  ...
  it('does not forward secrets or execution-injection environment variables', () => {
    assert.deepEqual(
      safeEnvironment({
        ...
        API_PORT: '3001',
        DB_PATH: '/private/user.sqlite',
        HTTP_PROXY: 'http://external-proxy.invalid',
        FANTASY_UI_ORIGIN: 'http://external.invalid',
        FANTASY_UI_SCENARIO: 'crash',
        PLAYWRIGHT_BROWSERS_PATH: '/user/browser',
        CI: 'true',
      }),
      { PATH: '/tools', CI: 'true' },
    );
