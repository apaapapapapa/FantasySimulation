...
export function safeEnvironment(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  const allowed = new Set(['PATH', 'HOME', 'USERPROFILE', 'SYSTEMROOT', 'WINDIR', 'COMSPEC',
    'TEMP', 'TMP', 'TMPDIR', 'APPDATA', 'LOCALAPPDATA', 'PATHEXT', 'CI', 'VP_HOME',
    'PNPM_HOME', 'VITE_PLUS_HOME', 'MIGRATION_BASE_SHA']);
...
export function runCommand(command: string, args: string[], cwd: string,
  ...
  if (!Number.isSafeInteger(timeoutMs) || timeoutMs <= 0 ||
      !Number.isSafeInteger(maxBytes) || maxBytes <= 0) throw new Error('Invalid process budget');
  return new Promise((resolve) => {
    const child = spawn(command, args, { cwd, shell: false, windowsHide: true,
      detached: process.platform !== 'win32', env: options.env ?? safeEnvironment(process.env),
    ...
    child.on('close', (exitCode, signal) => {
      clearTimeout(timer);
      resolve({ exitCode, signal, bounded, output: redact(Buffer.concat(chunks).toString('utf8'), process.env) });
    });
  });
}
