...
export function git(root: string, args: string[]): string {
  return execFileSync('git', args, { cwd: root, encoding: 'utf8', timeout: 30000,
    maxBuffer: 8 * 1024 * 1024, stdio: ['ignore', 'pipe', 'pipe'] }).trimEnd();
}
export function artifactDirectory(root: string, relative: string): string {
  if (!/^\.generated\/harness\/[\w.-]+(?:\/[\w.-]+)*$/.test(relative) ||
...
export function sourceIdentity(root: string, env: NodeJS.ProcessEnv = process.env) {
  ...
  if (env.GITHUB_SHA && env.GITHUB_SHA !== sourceSha) throw new Error('Checkout differs from CI source');
...
export async function collectSource(root: string, relative: string,
  run: typeof runCommand = runCommand, env: NodeJS.ProcessEnv = process.env,
) {
  ...
  const result = before ? null : await run('vp', ['run', 'verify'], root);
  ...
  writeFileSync(join(directory, 'verify.log'), result?.output ?? 'Not run: checkout was not clean.\n');
  writeFileSync(join(directory, 'command.json'), JSON.stringify({ ...info,
    command: ['vp', 'run', 'verify'], platform: process.platform, nodeVersion: process.version,
    startedAt, finishedAt: new Date().toISOString(), exitCode: result?.exitCode ?? null,
    signal: result?.signal ?? null, bounded: result?.bounded ?? false,
    cleanBefore: !before, cleanAfter: !after }, null, 2) + '\n');
  const report: Report = { ...info, schemaVersion: 1, producer: 'source-runner', startedAt,
    finishedAt: new Date().toISOString(), checks: [
      { id: 'source-clean', required: true, status: unchanged ? 'pass' : 'fail',
        reason: unchanged ? 'Clean, unchanged source tree' : 'Dirty checkout or source changed during verification',
        evidence: [{ uri: `${relative}/command.json`, sourceSha }] },
      { id: 'source-verify', required: true,
        status: result === null ? 'unknown' : result.exitCode === 0 && !result.bounded ? 'pass' : 'fail',
        reason: result === null ? 'Verification not run' : `vp run verify exit=${result.exitCode}; bounded=${result.bounded}`,
        evidence: [{ uri: `${relative}/verify.log`, sourceSha }, { uri: `${relative}/command.json`, sourceSha }] },
    ] };
